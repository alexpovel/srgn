resource "aws___T__instance" "__T__example" {
  ami           = "ami-__T__0c55b159cbfafe1f0"
  instance_type = "t2.micro"
}

resource "aws_instance__T__" "example__T__" {
...
resource "aws_s3___T__bucket" "example__T___bucket" {
...
resource "__T__aws_instance" "example_instance" {
  ...
  user_data = <<-EOF
              #!/bin/bash
              echo "BUCKET_NAME=${aws_s3___T__bucket.example__T___bucket.bucket}" > /etc/environment
              EOF
